  ...
  <div class="tab1 tab1b main">
    ...
    <div class="content">
        ...
        <table class="ordertab">
          ...
          <tr>
            <td class="left">
              <a href="<?php echo $GLOBALS['WWW'] ?>index.php?c=message&a=type&tid=23&fmolds=<?php echo $molds ?>&faid=<?php echo $channel['id'] ?>" style="color:#C00;font-size:14px;"><strong>【在线提交简历】</strong></a></td>
          </tr>
        </table>
        </div>
...
function comment_ret(){
	...
	if(document.comment.body.value.length>500){alert("评论内容不能超过500字！");return false;}
         ...
         <dt>验证码:</dt><dd><input name="vercode" type="text" style="width:50px;" class="inp" /></dd><dd><img src="<?php echo $GLOBALS['WWW'] ?>include/vercode.php" id="vercodeimg" width="58" height="24" onclick="newvercode();" style="cursor:pointer;" /></dd><?php }else{ ?><dt>&nbsp;</dt><?php } ?>
         <dd><input name="" type="submit" value="提交" class="btnmini" /></dd></dl>
         </form>
         </div>
         <script type="text/javascript">ajax_comment('comment','<?php echo $molds ?>',<?php echo $channel['id'] ?>,'person/ajax_comment.html');</script>
         <div id="comment"></div>
       </div>
    </div>
  </div>
