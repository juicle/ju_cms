...
class db_mysql {
	...
	public function __construct($dbConfig)
	{
		...
		if(!$this->conn || !mysql_select_db($dbConfig['database'], $this->conn)){
			echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />数据库无法链接，如果您是第一次使用，请先执行<br /><br />';exit;
		}
	...
	{
		if( TRUE != $GLOBALS['G_Ji']['db']['persistent'] )@mysql_close($this->conn);
	}
}
