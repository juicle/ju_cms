...
class product extends syController
{
	function __construct(){
		...
		$this->db=$GLOBALS['G_Ji']['db']['prefix'].'product';
	}
	function index(){
		...
		$this->attribute_type=syDB('product')->findSql('select distinct a.tid,a.aid,b.tid,b.isshow,b.orders,b.name from '.$this->db.'_attribute a left join '.$GLOBALS['G_Ji']['db']['prefix'].'attribute_type b on (a.tid=b.tid and a.aid='.$this->product['id'].') where b.isshow=1 order by b.orders desc,b.tid desc');
		...
		$this->record=total_page($GLOBALS['G_Ji']['db']['prefix'].'sales_record where aid='.$this->product['id']);
		$body=array_filter(explode("[ju|page]",$this->product['body']));
		if(count($body)>1){
			$pages=array(
